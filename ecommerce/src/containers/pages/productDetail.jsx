...
const ProductDetail = ({
    ...
    const addToCart = async () => {
        if (product && product !== null && product !== undefined && product.count > 0) {
            setLoading(true)
    ...
    const {head, body, rating} = formData
    ...
    const leaveReview = e => {
        e.preventDefault()
        if (rating !== null)
            create_review(productId, rating, head, body);
    }

    const updateReview = e => {
        e.preventDefault()
        if (rating !== null)
            update_review(productId, rating, head, body);
    }
